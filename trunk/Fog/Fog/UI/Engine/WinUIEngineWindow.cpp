...
err_t WinUIEngineWindowImpl::create(uint32_t hints)
{
  ...
  else
  {
    ...
    {
      Debug::dbgFunc("Fog::WinUIEngineWindowImpl", "create", "CreateWindowExW() failed\n.");
      goto _Fail;
    }
...
err_t WinUIEngineWindowImpl::worldToClient(PointI& pt) const
{
  ...
  if (::ScreenToClient(static_cast<HWND>(_handle), (POINT *)&pt))
    ...
    return ERR_UI_CANT_TRANSLATE_COORDINATES;
}
...
err_t WinUIEngineWindowImpl::setOpacity(float opacity)
{
  if (FOG_IS_NULL(_handle))
  ...
  switch (msg)
  {
    ...
    case WM_PAINT:
    {
      ...
      else
      {
        ...
        _regionOfInterest.union_(BoxI(0, 0, cr.right - cr.left, cr.bottom - cr.top));
        _shouldBlit = true;
      }

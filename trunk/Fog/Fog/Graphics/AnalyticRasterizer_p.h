...
#include <Fog/Core/Pack/PackRestore.h>
  ...
  FOG_INLINE void setClipBox(const BoxI& clipBox) { _clipBox = clipBox; }
  ...
  typedef Span8* (*SweepScanlineRegionFn)(AnalyticRasterizer8* rasterizer, Scanline8& scanline, MemoryBuffer& temp, int y, const BoxI* clipBoxes, sysuint_t count);
  ...
  //! [5, 5, 10, 10] then @c _offset will be set to [-5 << 8, -5 << 8].
  PointI _offset24x8;

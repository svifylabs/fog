...
void GuiEngine::doUpdateWindow(GuiWindow* window)
{
...

  {
    ...
    if (window->_backingStore->getSize() != topSize)
    {
  ...
  // The Painter::begin() method is called here, because it is shared
  ...
  // want to do that).
  {
    ...
    imageBits.size.w = Math::min<int>(window->_backingStore->getSize().getWidth(), topSize.getWidth());
    imageBits.size.h = Math::min<int>(window->_backingStore->getSize().getHeight(), topSize.getHeight());
    imageBits.format = window->_backingStore->getFormat();
    imageBits.stride = window->_backingStore->getStride();
    imageBits.data = window->_backingStore->getPixels();
    painter.begin(imageBits, NO_FLAGS /*| PAINTER_INIT_MT */);
  }
...
inside:
    ...
    if (parent)
    {
      widgetRec.bounds.x0 = parentRec.bounds.getX0() + widget->getX();
      widgetRec.bounds.y0 = parentRec.bounds.getY0() + widget->getY();
      widgetRec.bounds.x1 = widgetRec.bounds.getX0() + widget->getWidth();
      widgetRec.bounds.y1 = widgetRec.bounds.getY0() + widget->getHeight();
    ...
    {
      widgetRec.bounds.x0 = 0;
      widgetRec.bounds.y0 = 0;
      widgetRec.bounds.x1 = widget->getWidth();
      widgetRec.bounds.y1 = widget->getHeight();
    ...
    if ((widgetRec.uflags & WIDGET_UPDATE_CHILD) != 0 && widget->_objectExtra->_children.getLength())
    {
      ...
      if (FOG_IS_ERROR(stackerr)) goto end;

...
struct FOG_HIDDEN EventLoopAutoRunState
{
...
EventLoop::EventLoop(const String& type) :
  _type(type),
  _depth(0),
  _quitting(0),
  _destroyedCalled(0)
...
bool EventLoop::_deletePendingTasks()
{
  ...
  {
    EventLoopPendingTask pendingTask = _workQueue.takeFirst();
  ...
  {
    Task* task = _deferredWorkQueue.takeFirst().getTask();
    if (task->destroyOnFinish()) task->destroy();
  }
  ...
  {
    Task* task =_delayedWorkQueue.takeFirst().getTask();
    if (task->destroyOnFinish()) task->destroy();
  }
...
bool EventLoop::_doDelayedWork(Time* nextDelayedWorkTime)
{
  ...
  Time t = _delayedWorkQueue.at(0).getTime();
  if (t > Time::now())
  {
    // Still not ellapsed.
  ...
  EventLoopPendingTask pendingTask = _delayedWorkQueue.takeFirst();

  if (!_delayedWorkQueue.isEmpty())
    *nextDelayedWorkTime = _delayedWorkQueue.at(0).getTime();
...
void EventLoop::_destroyed()
{
  ...
  if (didWork)
  {
    fog_debug("Fog::EventLoop::_destroyed() - Couldn't delete all tasks.");
  }
...
void WinEventLoop::pumpOutPendingPaintMessages()
{
  ...
  bool win2k = OS::getWindowsVersion() <= OS::WIN_VERSION_2000;
  int peekCount;
  MSG msg;

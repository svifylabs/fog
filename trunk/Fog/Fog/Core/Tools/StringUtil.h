...
{
  _api.stringutil_copyA(dst, src, length);
}

static FOG_INLINE void copy(uchar* dst, const uchar* src, size_t length)
{
  _api.stringutil_copyA(reinterpret_cast<char*>(dst), reinterpret_cast<const char*>(src), length);
}

static FOG_INLINE void copy(CharW* dst, const char* src, size_t length)
{
  _api.stringutil_unicodeFromLatin(dst, src, length);
}

static FOG_INLINE void copy(uint16_t* dst, const uchar* src, size_t length)
{
  _api.stringutil_unicodeFromLatin(reinterpret_cast<CharW*>(dst), reinterpret_cast<const char*>(src), length);
}

static FOG_INLINE void copy(CharW* dst, const CharW* src, size_t length)
{
  _api.stringutil_copyW(dst, src, length);
}

static FOG_INLINE void copy(uint16_t* dst, const uint16_t* src, size_t length)
{
  _api.stringutil_copyW(reinterpret_cast<CharW*>(dst), reinterpret_cast<const CharW*>(src), length);
}
...
{
  return _api.stringutil_latinFromUnicode(dst, src, length);
}

static FOG_INLINE err_t latinFromUnicode(uchar* dst, const uint16_t* src, size_t length)
{
  return _api.stringutil_latinFromUnicode(reinterpret_cast<char*>(dst), reinterpret_cast<const CharW*>(src), length);
}
...
{
  _api.stringutil_moveA(dst, src, length);
}

static FOG_INLINE void move(uchar* dst, const uchar* src, size_t length)
{
  _api.stringutil_moveA(reinterpret_cast<char*>(dst), reinterpret_cast<const char*>(src), length);
}

static FOG_INLINE void move(CharW* dst, const CharW* src, size_t length)
{
  _api.stringutil_moveW(dst, src, length);
}

static FOG_INLINE void move(uint16_t* dst, const uint16_t* src, size_t length)
{
  _api.stringutil_moveW(reinterpret_cast<CharW*>(dst), reinterpret_cast<const CharW*>(src), length);
}
...
{
  _api.stringutil_fillA(dst, ch, length);
}

static FOG_INLINE void fill(uchar* dst, uchar ch, size_t length)
{
  _api.stringutil_fillA(reinterpret_cast<char*>(dst), ch, length);
}

static FOG_INLINE void fill(CharW* dst, CharW ch, size_t length)
{
  _api.stringutil_fillW(dst, ch._value, length);
}

static FOG_INLINE void fill(uint16_t* dst, uint16_t ch, size_t length)
{
  _api.stringutil_fillW(reinterpret_cast<CharW*>(dst), ch, length);
}
...
{
  return _api.stringutil_lenA(src);
}

static FOG_INLINE size_t len(const uchar* src)
{
  return _api.stringutil_lenA(reinterpret_cast<const char*>(src));
}

static FOG_INLINE size_t len(const CharW* src)
{
  return _api.stringutil_lenW(src);
}

static FOG_INLINE size_t len(const uint16_t* src)
{
  return _api.stringutil_lenW(reinterpret_cast<const CharW*>(src));
}

static FOG_INLINE size_t nlen(const char* src, size_t max)
{
  return _api.stringutil_nLenA(src, max);
}

static FOG_INLINE size_t nlen(const uchar* src, size_t max)
{
  return _api.stringutil_nLenA(reinterpret_cast<const char*>(src), max);
}

static FOG_INLINE size_t nlen(const CharW* src, size_t max)
{
  return _api.stringutil_nLenW(src, max);
}

static FOG_INLINE size_t nlen(const uint16_t* src, size_t max)
{
  return _api.stringutil_nLenW(reinterpret_cast<const CharW*>(src), max);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfStringWA[cs](reinterpret_cast<const CharW*>(aStr), aLength, reinterpret_cast<const char*>(bStr), bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfStringW[cs](reinterpret_cast<const CharW*>(aStr), aLength, reinterpret_cast<const CharW*>(bStr), bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfAnyA[cs](str, length, charArray, charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfAnyA[cs](reinterpret_cast<const char*>(str), length, reinterpret_cast<const char*>(charArray), charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfAnyW[cs](str, length, charArray, charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_indexOfAnyW[cs](reinterpret_cast<const CharW*>(str), length, reinterpret_cast<const CharW*>(charArray), charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringA[cs](aStr, aLength, bStr, bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringA[cs](reinterpret_cast<const char*>(aStr), aLength, reinterpret_cast<const char*>(bStr), bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringWA[cs](aStr, aLength, bStr, bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringW[cs](aStr, aLength, bStr, bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringWA[cs](reinterpret_cast<const CharW*>(aStr), aLength, reinterpret_cast<const char*>(bStr), bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfStringW[cs](reinterpret_cast<const CharW*>(aStr), aLength, reinterpret_cast<const CharW*>(bStr), bLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfAnyA[cs](str, length, charArray, charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfAnyA[cs](reinterpret_cast<const char*>(str), length, reinterpret_cast<const char*>(charArray), charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfAnyW[cs](str, length, charArray, charLength);
}
...
{
  FOG_ASSERT(cs < CASE_SENSITIVITY_COUNT);
  return _api.stringutil_lastIndexOfAnyW[cs](reinterpret_cast<const CharW*>(str), length, reinterpret_cast<const CharW*>(charArray), charLength);
}
...
{
  return _api.stringutil_validateUtf8(data, length, invalid);
}

static FOG_INLINE err_t validateUtf8(const uchar* data, size_t length, size_t* invalid)
{
  return _api.stringutil_validateUtf8(reinterpret_cast<const char*>(data), length, invalid);
}

static FOG_INLINE err_t validateUtf16(const CharW* data, size_t length, size_t* invalid)
{
  return _api.stringutil_validateUtf16(data, length, invalid);
}

static FOG_INLINE err_t validateUtf16(const uint16_t* data, size_t length, size_t* invalid)
{
  return _api.stringutil_validateUtf16(reinterpret_cast<const CharW*>(data), length, invalid);
}
...
{
  return _api.stringutil_ucsFromUtf8Length(data, length, ucsLength);
}

static FOG_INLINE err_t ucsFromUtf8Length(const uchar* data, size_t length, size_t* ucsLength)
{
  return _api.stringutil_ucsFromUtf8Length(reinterpret_cast<const char*>(data), length, ucsLength);
}

static FOG_INLINE err_t ucsFromUtf16Length(const CharW* data, size_t length, size_t* ucsLength)
{
  return _api.stringutil_ucsFromUtf16Length(data, length, ucsLength);
}

static FOG_INLINE err_t ucsFromUtf16Length(const uint16_t* data, size_t length, size_t* ucsLength)
{
  return _api.stringutil_ucsFromUtf16Length(reinterpret_cast<const CharW*>(data), length, ucsLength);
}
...
{
  return _api.stringutil_parseBoolA(dst, str, length, pEnd, pFlags);
}

static FOG_INLINE err_t parseBool(bool* dst, const CharW* str, size_t length, size_t* pEnd = NULL, uint32_t* pFlags = NULL)
{
  return _api.stringutil_parseBoolW(dst, str, length, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI8A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI8W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU8A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU8W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI16A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI16W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU16A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU16W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI32A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI32W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU32A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU32W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI64A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseI64W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU64A(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseU64W(dst, str, length, base, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseFloatA(dst, str, length, decimalPoint, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseFloatW(dst, str, length, decimalPoint, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseDoubleA(dst, str, length, decimalPoint, pEnd, pFlags);
}
...
{
  return _api.stringutil_parseDoubleW(dst, str, length, decimalPoint, pEnd, pFlags);
}

...
static const uint8_t Memory_protectedPattern[16] =
{
...
void MemDbg::insert(void* p, size_t size)
{
  ...
  memcpy(reinterpret_cast<char*>(p) + 32, Memory_protectedPattern, 16);
  memcpy(reinterpret_cast<char*>(p) + 48, Memory_protectedPattern, 16);

  // Copy the pattern to detect buffer overrun.
  memcpy(reinterpret_cast<char*>(p) + node->size + 64, Memory_protectedPattern, 16);
  memcpy(reinterpret_cast<char*>(p) + node->size + 80, Memory_protectedPattern, 16);
...
void MemDbg::check(void* p)
{
  ...
  if (memcmp(reinterpret_cast<char*>(p) + 32, Memory_protectedPattern, 16) != 0 ||
      memcmp(reinterpret_cast<char*>(p) + 48, Memory_protectedPattern, 16) != 0 )
  {
    Debug::failFunc("Fog::MemDbg", "check", "Detected buffer underrun (%p).", _USER_FROM_DBG(p));
  }

  // Check for the buffer overrun.
  if (memcmp(reinterpret_cast<char*>(p) + node->size + 64, Memory_protectedPattern, 16) != 0 ||
      memcmp(reinterpret_cast<char*>(p) + node->size + 80, Memory_protectedPattern, 16) != 0 )
  {
...
static void* FOG_CDECL Memory_alloc_debug(size_t size)
{
...
static void* FOG_CDECL Memory_calloc_debug(size_t size)
{
  AutoLock locked(memdbg->lock);
...
static void* FOG_CDECL Memory_realloc_debug(void* p, size_t size)
{
  AutoLock locked(memdbg->lock);
  void* newp;
...
static void FOG_CDECL Memory_free_debug(void* p)
{
  AutoLock locked(memdbg->lock);
  if (p == NULL) return;
  p = _DBG_FROM_USER(p);
...
FOG_NO_EXPORT void _core_memoryalloc_init_debug(void)
{
  memdbg.init();

  // Replace the default allocators debug versions.
  _api.memory._m_alloc = Memory_alloc_debug;
  _api.memory._m_calloc = Memory_calloc_debug;
  _api.memory._m_realloc = Memory_realloc_debug;
  _api.memory._m_free = Memory_free_debug;
}

FOG_NO_EXPORT void _core_memoryalloc_fini_debug(void)
{
  memdbg->show();
  memdbg.destroy();
}

...
// [Guard]
#ifndef _FOG_CORE_MEMORYBUFFER_H
#define _FOG_CORE_MEMORYBUFFER_H
...
//! use @c MemoryBuffer as an input parameter to some function that needs 
//! always temporary memory in various size.
//!
//! The @c MemoryBuffer class allows to embed statically allocated buffer that
//! can be used instead of dynamically allocated one. The embedded buffer can
//! be allocated on the stack (using @c TemporaryMemoryBuffer template) or
//! pre-allocated by you. Using embedded buffer should be benefical in heavy
//! multithreaded environment while processing small chunks of data.
//!
...
struct FOG_API MemoryBuffer
{
  ...
  FOG_INLINE uint8_t* alloc(sysuint_t size) { return (size > _memorySize) ? _alloc(size) : _memoryBuffer; }

protected:
  //! @brief Private allocator used if there is no memory. This method is not
  //! inlined, because it is called only if buffer is too small.
...
// [Guard]
#endif // _FOG_CORE_MEMORYBUFFER_H

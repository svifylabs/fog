...
RasterPaintWorkTask::RasterPaintWorkTask(Lock* lock) :
  cond(lock)
{
...
RasterPaintWorkTask::~RasterPaintWorkTask()
{
}

void RasterPaintWorkTask::run()
{
  RasterPaintEngine* engine = ctx.engine;
  RasterPaintWorkMgr* wm = engine->wm;

#if defined(FOG_DEBUG_RASTER_SYNC)
  Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - ThreadID=%d.\n", ctx.id, Thread::getCurrentThreadId());
    ...
    do {
      ...
      while (calcCurrent < wm->calcPosition)
      {
        ...
        if (clc && AtomicCore<RasterCalc*>::cmpXchg(pclc, clc, NULL))
        {
          ...
          Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - Calculation %d (%p)\n", ctx.id, (int)calcCurrent, clc);
      ...
      while (cmdCurrent < wm->cmdPosition)
      {
        ...
        Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - command %d (%p) status=%s\n", ctx.id, (int)cmdCurrent, cmd, statusMsg[cmd->status.get()]);
#endif // FOG_DEBUG_RASTER_CMD
...
skipCommands:
...

    {
...
#if defined(FOG_DEBUG_RASTER_SYNC)
      Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - Going to wait (currently waiting=%d)\n",
        ctx.id,
      ...
      if (wm->waitingWorkers.addXchg(1) + 1 == wm->numWorkers)
      {
        ...
        {
#if defined(FOG_DEBUG_RASTER_SYNC)
          Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - Everything done, signaling allFinished...\n", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
      ...
      if (cmdCurrent == wm->cmdPosition)
      {
        ...
        {
  #if defined(FOG_DEBUG_RASTER_SYNC)
          Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::run() - Quitting...\n", ctx.id);
  #endif // FOG_DEBUG_RASTER_SYNC
...
void RasterPaintWorkTask::destroy()
{
  RasterPaintEngine* engine = ctx.engine;
  RasterPaintWorkMgr* wm = engine->wm;

#if defined(FOG_DEBUG_RASTER_SYNC)
  Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::destroy()\n", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
  ...
  {
#if defined(FOG_DEBUG_RASTER_SYNC)
    Debug::dbgMessage("Fog::RasterPaintWorkTask[#%d]::destroy() - I'm last, signaling release event...\n", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
...
RasterPaintWorkMgr::RasterPaintWorkMgr() :
  numWorkers(0),
  maxWorkers(0),
...
RasterPaintWorkTask* RasterPaintWorkMgr::wakeUpScheduled(RasterPaintWorkTask* calledFrom)
{
  size_t c = cmdCount.get();

  for (size_t i = 0; i < numWorkers; i++)
  {
    RasterPaintWorkTask* worker = &workers[i];
...
RasterPaintWorkTask* RasterPaintWorkMgr::wakeUpSleeping(RasterPaintWorkTask* calledFrom)
{
  for (size_t i = 0; i < numWorkers; i++)
  {
    RasterPaintWorkTask* worker = &workers[i];
...
bool RasterPaintWorkMgr::isCompleted()
{
  size_t done = 0;
  size_t c = cmdCount.get();

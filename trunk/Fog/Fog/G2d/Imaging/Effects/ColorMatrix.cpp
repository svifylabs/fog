...
static const float ColorMatrix_oIdentity[25] =
{
...
static const float ColorMatrix_oZero[25] =
{
...
static const float ColorMatrix_oGreyscale[25] =
{
...
static const float ColorMatrix_oPreHue[25] =
{
...
static const float ColorMatrix_oPostHue[25] =
{
...
{
  _api.colormatrix.copy(self->m, ColorMatrix_oIdentity);
}
...
static err_t FOG_CDECL ColorMatrix_multiplyOther(ColorMatrix* dst, const ColorMatrix* other, uint32_t order)
{
  if (order == MATRIX_ORDER_PREPEND)
    return _api.colormatrix.multiplyMatrix(dst, other, dst);
  else
    return _api.colormatrix.multiplyMatrix(dst, dst, other);
}
...
static err_t FOG_CDECL ColorMatrix_multiplyMatrix(ColorMatrix* dst, const ColorMatrix* a, const ColorMatrix* b)
{
  ...
  if (a == dst || b == dst)
  {
    _api.colormatrix.copy(tmp, dst->m);
    if (a == dst) am = tmp;
    if (b == dst) bm = tmp;
  }
...
static err_t FOG_CDECL ColorMatrix_rotateHue(ColorMatrix* self, float phi)
{
  ...
  _api.colormatrix.simplifiedPremultiply(self,
...
static err_t FOG_CDECL ColorMatrix_rotateColor(ColorMatrix* self, int x, int y, float phi, uint32_t order)
{
  ...
  if (Math::max(x, y) < 3 && order == MATRIX_ORDER_PREPEND)
    return _api.colormatrix.simplifiedPremultiply(self, &rot);
  else
    return _api.colormatrix.multiplyOther(self, &rot, order);
}
...
static err_t FOG_CDECL ColorMatrix_shearColor(ColorMatrix* self, int x, int y0, float c0, int y1, float c1, uint32_t order)
{
  ...
  if (Math::max(x, y0, y1) < 3 && order == MATRIX_ORDER_PREPEND)
    return _api.colormatrix.simplifiedPremultiply(self, &shear);
  else
    return _api.colormatrix.multiplyOther(self, &shear, order);
}
...
FOG_NO_EXPORT void ColorMatrix_init(void)
{
  ...
  _api.colormatrix.ctor = ColorMatrix_ctor;
  _api.colormatrix.getType = ColorMatrix_getType;

  _api.colormatrix.addMatrix = ColorMatrix_addMatrix;
  _api.colormatrix.addScalar = ColorMatrix_addScalar;
  _api.colormatrix.subtractMatrix = ColorMatrix_subtractMatrix;
  _api.colormatrix.subtractScalar = ColorMatrix_subtractScalar;
  _api.colormatrix.multiplyOther = ColorMatrix_multiplyOther;
  _api.colormatrix.multiplyMatrix = ColorMatrix_multiplyMatrix;
  _api.colormatrix.multiplyScalar = ColorMatrix_multiplyScalar;
  _api.colormatrix.simplifiedPremultiply = ColorMatrix_simplifiedPremultiply;

  _api.colormatrix.translateArgb = ColorMatrix_translateArgb;
  _api.colormatrix.scaleArgb = ColorMatrix_scaleArgb;
  _api.colormatrix.scaleTint = ColorMatrix_scaleTint;
  _api.colormatrix.saturate = ColorMatrix_saturate;
  _api.colormatrix.rotateHue = ColorMatrix_rotateHue;
  _api.colormatrix.rotateColor = ColorMatrix_rotateColor;
  _api.colormatrix.shearColor = ColorMatrix_shearColor;

  _api.colormatrix.mapArgb32 = ColorMatrix_mapArgb32;
  _api.colormatrix.mapArgb64 = ColorMatrix_mapArgb64;
  _api.colormatrix.mapArgbF = ColorMatrix_mapArgbF;

  _api.colormatrix.copy = ColorMatrix_copy;
  _api.colormatrix.eq = ColorMatrix_eq;

  // --------------------------------------------------------------------------
  // [Data]
  // --------------------------------------------------------------------------

  _api.colormatrix.oIdentity = reinterpret_cast<const ColorMatrix*>(ColorMatrix_oIdentity);
  _api.colormatrix.oZero = reinterpret_cast<const ColorMatrix*>(ColorMatrix_oZero);
  _api.colormatrix.oGreyscale = reinterpret_cast<const ColorMatrix*>(ColorMatrix_oGreyscale);
  _api.colormatrix.oPreHue = reinterpret_cast<const ColorMatrix*>(ColorMatrix_oPreHue);
  _api.colormatrix.oPostHue = reinterpret_cast<const ColorMatrix*>(ColorMatrix_oPostHue);
}

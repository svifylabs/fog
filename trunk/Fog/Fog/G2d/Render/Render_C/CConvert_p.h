...
// [Guard]
#ifndef _FOG_G2D_RENDER_RENDER_C_CCONVERT_P_H
#define _FOG_G2D_RENDER_RENDER_C_CCONVERT_P_H
...
struct FOG_NO_EXPORT CConvert
{
  ...
  static err_t FOG_FASTCALL init(ImageConverterData* d)
  {
    ...
    if (d->isCopy)
    {
      ...
      switch (df.getDepth())
      {
        case   8: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_8  ]; return ERR_OK;
        case  16: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_16 ]; return ERR_OK;
        case  24: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_24 ]; return ERR_OK;
        case  32: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_32 ]; return ERR_OK;
        case  48: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_48 ]; return ERR_OK;
        case  64: d->blitFn = _g2d_render.convert.copy[RENDER_CONVERTER_COPY_64 ]; return ERR_OK;
      }
    }
    ...
    if (d->isBSwap)
    {
      ...
      switch (df.getDepth())
      {
        case  16: d->blitFn = _g2d_render.convert.bswap[RENDER_CONVERTER_BSWAP_16  ]; return ERR_OK;
        case  24: d->blitFn = _g2d_render.convert.bswap[RENDER_CONVERTER_BSWAP_24  ]; return ERR_OK;
        case  32: d->blitFn = _g2d_render.convert.bswap[RENDER_CONVERTER_BSWAP_32  ]; return ERR_OK;
        case  48: d->blitFn = _g2d_render.convert.bswap[RENDER_CONVERTER_BSWAP_48  ]; return ERR_OK;
        case  64: d->blitFn = _g2d_render.convert.bswap[RENDER_CONVERTER_BSWAP_64  ]; return ERR_OK;
      }
    }

    // Prepare the structure.
    RenderConverterMulti* multi = reinterpret_cast<RenderConverterMulti*>(d->buffer);
    memset(multi, 0, sizeof(RenderConverterMulti));
    ...
    if (dfMaxBpc <= 8 && sfMaxBpc <= 8)
    {
      ...
      if (sf.getFormat() == IMAGE_FORMAT_PRGB32 || sf.getFormat() == IMAGE_FORMAT_XRGB32)
      {
        ...
        else
        {
          ...
          multi->step = RENDER_CONVERTER_BUFFER_SIZE / 4;
    ...
    if (dfMaxBpc <= 16 && sfMaxBpc <= 16)
    {
      ...
      if (sf.getFormat() == IMAGE_FORMAT_PRGB64)
      {
        ...
        {
          // One pass run, but including pre-process (color-space conversion).
          multi->step = RENDER_CONVERTER_BUFFER_SIZE / 8;
          multi->dstAdvance = df.getBytesPerPixel() * multi->step;
          multi->srcAdvance = sf.getBytesPerPixel() * multi->step;
          multi->blit[0] = blitLine;
          d->blitFn = pass_one_preprocess;
  ...
  static uint32_t getDibFormat(const ImageFormatDescription& d)
  {
    ...
    {
      case 8:
        if (d.getFormat() == IMAGE_FORMAT_A8) return RENDER_CONVERTER_DIB_A8;
        if (d.getFormat() == IMAGE_FORMAT_I8) return RENDER_CONVERTER_DIB_I8;
        break;

      case 16:
        if (d.getAMask32() == 0xFFFF)
          return RENDER_CONVERTER_DIB_A16 + d.isByteSwapped();
        else if (d.getAMask32() == 0x0000 && d.getRMask32() == 0x7C00 && d.getGMask32() == 0x03E0 && d.getBMask32() == 0x001F)
          return RENDER_CONVERTER_DIB_RGB16_555 + d.isByteSwapped();
        else if (d.getAMask32() == 0x0000 && d.getRMask32() == 0xF800 && d.getGMask32() == 0x07E0 && d.getBMask32() == 0x001F)
          return RENDER_CONVERTER_DIB_RGB16_565 + d.isByteSwapped();
        else if (d.getAMask32() == 0xF000 && d.getRMask32() == 0x0F00 && d.getGMask32() == 0x00F0 && d.getBMask32() == 0x000F)
          return RENDER_CONVERTER_DIB_ARGB16_4444 + d.isByteSwapped();
        else
          return RENDER_CONVERTER_DIB_ARGB16_DIB + d.isByteSwapped();

      case 24:
        if (d.getAMask32() == 0 && d.getRMask32() == 0x00FF0000 && d.getGMask32() == 0x0000FF00 && d.getBMask32() == 0x000000FF)
          return RENDER_CONVERTER_DIB_RGB24_888;
        else if (d.getAMask32() == 0 && d.getRMask32() == 0x000000FF && d.getGMask32() == 0x0000FF00 && d.getBMask32() == 0x00FF0000)
          return RENDER_CONVERTER_DIB_RGB24_888_BS;
        else
          return RENDER_CONVERTER_DIB_ARGB24_DIB + d.isByteSwapped();

      case 32:
        if (d.getAMask32() == 0 && d.getRMask32() == 0x00FF0000 && d.getGMask32() == 0x0000FF00 && d.getBMask32() == 0x000000FF)
          return RENDER_CONVERTER_DIB_RGB32_888;
        else if (d.getAMask32() == 0 && d.getRMask32() == 0x000000FF && d.getGMask32() == 0x0000FF00 && d.getBMask32() == 0x00FF0000)
          return RENDER_CONVERTER_DIB_RGB32_888_BS;
        else if (d.getAMask32() == 0xFF000000 && d.getRMask32() == 0x00FF0000 && d.getGMask32() == 0x0000FF00 && d.getBMask32() == 0x000000FF)
          return RENDER_CONVERTER_DIB_ARGB32_8888;
        else if (d.getAMask32() == 0x000000FF && d.getRMask32() == 0x0000FF00 && d.getGMask32() == 0x00FF0000 && d.getBMask32() == 0xFF000000)
          return RENDER_CONVERTER_DIB_ARGB32_8888_BS;
        else
      ...
      case 48:
        if (d.getAMask64() == 0 && d.getRMask64() == FOG_UINT64_C(0x0000FFFF00000000) && 
          ...
          return RENDER_CONVERTER_DIB_RGB48_DIB + d.isByteSwapped();
        else
      ...
      case 64:
        if (d.getAMask64() == FOG_UINT64_C(0xFFFF000000000000) && 
          ...
          return RENDER_CONVERTER_DIB_ARGB64_DIB + d.isByteSwapped();
    }

    // Failed, shouldn't happen.
    return RENDER_CONVERTER_DIB_COUNT;
  }
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    uint8_t buffer[RENDER_CONVERTER_BUFFER_SIZE];
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p32Load2aNative(pix0p, src);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p32Load2aSwap(pix0p, src);
      Face::p32FRGB32FromRGB16_555(pix0p, pix0p);
      Face::p32Store4aNative(dst, pix0p);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p32Load2aNative(pix0r, src + 0);
      Face::p32Load2aNative(pix0g, src + 2);
      Face::p32Load2aNative(pix0b, src + 4);
#else
      Face::p32Load2aNative(pix0b, src + 0);
      Face::p32Load2aNative(pix0g, src + 2);
      Face::p32Load2aNative(pix0r, src + 4);
#endif // FOG_BYTE_ORDER

      Face::p32Store4aNative(dst, _FOG_FACE_COMBINE_4(0xFF000000, pix0r << 16, (pix0g & 0xFF) << 8, (pix0b & 0xFF)));
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    do {
      ...
      Face::p32Store4aNative(dst, _FOG_FACE_COMBINE_4(pix0a, pix0r, pix0g, pix0b));
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    do {
      ...
      Face::p32Load3bNative(pix0p, src);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    do {
      ...
      Face::p32Load4aNative(pix0p, src);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    do {
      ...
      Face::p64Load8aNative(pix0p, src);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p32Load2aSwap(pix0p_10, src);
      Face::p32FRGB64FromRGB16_555(pix0p_10, pix0p_32, pix0p_10);
      Face::p32Store8aNative(dst, pix0p_10, pix0p_32);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p32Load4aSwap(pix0p_10, src);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    do {
      ...
      Face::p64Store8aNative(dst, pix0p);
    ...
    uint8_t* dst, const uint8_t* src, int w, const RenderClosure* closure)
  {
    ...
    do {
      ...
      Face::p32Load2aNative(pix0p_10, src);
...
struct FOG_NO_EXPORT DibC
{
  ...
  static void FOG_FASTCALL argb32_from_prgb32(
    uint8_t* _dst, const uint8_t* _src, int w, const RenderClosure* closure)
  {
...
#endif // _FOG_G2D_RENDER_RENDER_C_CCONVERT_P_H

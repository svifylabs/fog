...
// [Guard]
#ifndef _FOG_G2D_RENDER_RENDER_C_TEXTUREBASE_P_H
#define _FOG_G2D_RENDER_RENDER_C_TEXTUREBASE_P_H
...
// ============================================================================
// [Fog::RenderC - Pattern - Texture]
// ============================================================================

#if 0

//! @internal
struct FOG_NO_EXPORT PatternTextureC
{
  // --------------------------------------------------------------------------
  // [Pattern - Texture - Formats]
  // --------------------------------------------------------------------------

  struct FOG_NO_EXPORT FMT_XRGB32
  {
    enum { BPP = 4, AS_IS = 1 };

    FOG_INLINE FMT_XRGB32(const RenderPatternContext* ctx) {}
    FOG_INLINE uint32_t fetch(const uint8_t* p) { return Memory::read_4(p) | 0xFF000000; }
  };

  struct FOG_NO_EXPORT FMT_ARGB32
  {
    enum { BPP = 4, AS_IS = 0 };

    FOG_INLINE FMT_ARGB32(const RenderPatternContext* ctx) {}
    FOG_INLINE uint32_t fetch(const uint8_t* p) { return ColorUtil::premultiply(Memory::read_4(p)); }
  };

  struct FOG_NO_EXPORT FMT_PRGB32
  {
    enum { BPP = 4, AS_IS = 1 };

    FOG_INLINE FMT_PRGB32(const RenderPatternContext* ctx) {}
    FOG_INLINE uint32_t fetch(const uint8_t* p) { return Memory::read_4(p); }
  };

  struct FOG_NO_EXPORT FMT_A8
  {
    enum { BPP = 1, AS_IS = 0 };

    FOG_INLINE FMT_A8(const RenderPatternContext* ctx) {}
    FOG_INLINE uint32_t fetch(const uint8_t* p) { return (uint32_t)p[0] << 24; }
  };

  struct FOG_NO_EXPORT FMT_I8
  {
    enum { BPP = 1, AS_IS = 0 };

    FOG_INLINE FMT_I8(const RenderPatternContext* ctx) { pal = ctx->texture.pal; }
    ...
    uint32_t spread, uint32_t interpolationType)
  {
    ...
    bool isTransformed = (!Math::isFuzzyEq(transform._00, 1.0) || !Math::isFuzzyEq(transform._01, 0.0) ||
                          !Math::isFuzzyEq(transform._11, 1.0) || !Math::isFuzzyEq(transform._10, 0.0) );
    ...
    switch (format)
    {
      ...
      {
        const ImagePalette& pal = ctx->texture.texture->getPalette();
    ...
    if (!isTransformed)
    {
      int64_t startx = Math::fixed48x16FromFloat(transform._20);
      int64_t starty = Math::fixed48x16FromFloat(transform._21);
    ...
    if (!ctx->fetch)
    {
      ...
      {
        int dx = Math::fixed16x16FromFloat(ctx->inv._00);
        int dy = Math::fixed16x16FromFloat(ctx->inv._01);

        int fxmax = ctx->texture.w << 16;
        int fymax = ctx->texture.h << 16;

        if (spread == GRADIENT_SPREAD_REFLECT)
        {
          fxmax *= 2;
          fymax *= 2;
        }
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    if ((uint)y >= (uint)th)
    {
      PatternBase::fetch_solid(span, buffer, mode, 0x00000000);
      return;
    }
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      if (x < 0)
      {
        ...
        dst = PatternBase::fill32(dst, 0x00000000, i);
        if (!w) goto fetchSkip;

        x = 0;
      }
      // Or set image buffer if span fits into it.
      else if (P_TEXTURE_FORMAT::AS_IS && mode == RENDER_FETCH_REFERENCE && 
                            x < tw && w < tw - x)
      {
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      if (x < 0)
      {
        ...
        dst = PatternBase::fill32(dst, c0, i);
        if (!w) goto fetchSkip;
      }
      // Or set image buffer if span fits into it.
      else if (P_TEXTURE_FORMAT::AS_IS && mode == RENDER_FETCH_REFERENCE && 
...
fetchSolidLoop:
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      int i = Math::min(tw - x, w);
      if (P_TEXTURE_FORMAT::AS_IS && mode == RENDER_FETCH_REFERENCE && 
                       w < tw - x)
      {
        P_FETCH_SPAN8_SET_CUSTOM(src);
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      if (x < 0)
      {
        ...
        dst = PatternBase::fill32(dst, back_0, i);
...
fetchFill:
        ...
        P_INTERPOLATE_C_32_2_WITH_ZERO(((uint32_t*)dst)[0], back_0, fY0X0)
        dst += 4;
        if (!--w) goto fetchSkip;
      }
...
fetchSolidLoop:
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      // Fetch the rest.
      if (w) dst = PatternBase::repeat32(dst, tw, w);
...
fetchSkip:
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    P_FETCH_SPAN8_BEGIN()
      ...
      // Fetch the rest.
      if (w) dst = PatternBase::repeat32(dst, tw2, w);
    ...
    if (y == -1 || y == th - 1)
    {
      ...
      P_FETCH_SPAN8_BEGIN()
        ...
        if (x < tw)
        {
          ...
          do {
            P_INTERPOLATE_C_32_2_WITH_ZERO(
              ((uint32_t*)dst)[0],
              tex.fetch(src), fY0X0)
    ...
    else
    {
      ...
      P_FETCH_SPAN8_BEGIN()
        ...
        if (x < 0)
        {
          ...
          uint32_t c0;
          P_INTERPOLATE_C_32_2(c0, back_0, fY0X0 + fY0X1, back_1, fY1X0 + fY1X1);
          dst = PatternBase::fill32(dst, c0, i);
          if (!w) goto fetchInnerSkip;
          goto fetchInnerFill;
        }
...
fetchInnerFill:
          ...
          do {
            P_INTERPOLATE_C_32_4(((uint32_t*)dst)[0],
              back_0, fY0X0,
              back_1, fY1X0,
              back_0 = tex.fetch(srcCur0), fY0X1,
              back_1 = tex.fetch(srcCur1), fY1X1);
        ...
        // Fetch the rest.
        P_INTERPOLATE_C_32_2(src0, back_0, fY0X0 + fY0X1, back_1, fY1X0 + fY1X1)
        goto fetchSolidLoop;
...
fetchSolidLoop:
    ...
    const RenderPatternContext* ctx, Span* span, uint8_t* buffer, int y, uint32_t mode)
  {
    ...
    int fx = Math::fixed16x16FromFloat(rx * ctx->inv._00 + ry * ctx->inv._10 + ctx->inv._20);
    int fy = Math::fixed16x16FromFloat(rx * ctx->inv._01 + ry * ctx->inv._11 + ctx->inv._21);
...
// [Guard]
#endif // _FOG_G2D_RENDER_RENDER_C_TEXTUREBASE_P_H

  ...
  const ImagePalette* srcPalette)
{
  ...
  if (df->getDepth() == sf->getDepth())
  {
    ...
    if (df->isPremultiplied() == sf->isPremultiplied())
    {
      ...
      {
        if ((depth == 24 && (df->getRMask32() == MemOps::bswap32(sf->getRMask32()) >> 8) &&
                            (df->getGMask32() == MemOps::bswap32(sf->getGMask32()) >> 8) &&
                            (df->getBMask32() == MemOps::bswap32(sf->getBMask32()) >> 8)) ||
            (depth == 32 && (df->getAMask32() == MemOps::bswap32(sf->getAMask32())     ) &&
                            (df->getRMask32() == MemOps::bswap32(sf->getRMask32())     ) &&
                            (df->getGMask32() == MemOps::bswap32(sf->getGMask32())     ) &&
                            (df->getBMask32() == MemOps::bswap32(sf->getBMask32())     )) )
        {
          d->isBSwap = true;
        }
      }

...
static void _PcxSwapHeader(PcxHeader* h)
{
  ...
  h->xMin          = MemOps::bswap16(h->xMin);
  h->yMin          = MemOps::bswap16(h->yMin);
  h->xMax          = MemOps::bswap16(h->xMax);
  h->yMax          = MemOps::bswap16(h->yMax);
  h->horizontalDPI = MemOps::bswap16(h->horizontalDPI);
  h->verticalDPI   = MemOps::bswap16(h->verticalDPI);
  h->bytesPerLine  = MemOps::bswap16(h->bytesPerLine);
  h->paletteInfo   = MemOps::bswap16(h->paletteInfo);
  h->hScreenSize   = MemOps::bswap16(h->hScreenSize);
  h->vScreenSize   = MemOps::bswap16(h->vScreenSize);
#endif
}

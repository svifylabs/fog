...
static FOG_INLINE FIXED WinFontFace_FIXEDZero()
{
  FIXED fx;
  fx.value = 0;
  fx.fract = 0;
...
static FOG_INLINE FIXED WinFontFace_FIXEDOne()
{
  FIXED fx;
  fx.value = 1;
  fx.fract = 0;
...
static FOG_INLINE FIXED WinFontFace_getFIXEDFrom16x16(uint32_t v16x16)
{
  FIXED fx;
  fx.value = v16x16 >> 16;
...
static FOG_INLINE FIXED WinFontFace_getFIXEDFromFloat(const NumT& d)
{
  return WinFontFace_getFIXEDFrom16x16(Math::fixed16x16FromFloat(d));
}

template<typename NumT>
static FOG_INLINE NumT WinFontFace_getFloatFromFIXED(const FIXED& f)
{
...
err_t WinFontContext::prepare(HFONT hFont)
{
  ...
  mat.eM11 = WinFontFace_FIXEDOne();
  mat.eM12 = WinFontFace_FIXEDZero();
  mat.eM21 = WinFontFace_FIXEDZero();
  mat.eM22 = WinFontFace_FIXEDOne();
  ...
  const uint8_t* glyphBuffer, size_t glyphSize, bool flipY)
{
  ...
  do {
    ...
    p1.x = WinFontFace_getFloatFromFIXED<NumT>(polyHeader->pfxStart.x);
    p1.y = WinFontFace_getFloatFromFIXED<NumT>(polyHeader->pfxStart.y);
    if (flipY) p1.y = -p1.y;
    ...
    while (polyCur < polyEnd)
    {
      ...
      switch (pc->wType)
      {
        case TT_PRIM_LINE:
        {
          ...
          {
            p1.x = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i].x);
            p1.y = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i].y);
            if (flipY) p1.y = -p1.y;
        ...
        case TT_PRIM_QSPLINE:
        {
          for (uint i = 0; i < (uint)pc->cpfx - 1; i++)
          {
            ...
            p2.x = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i + 1].x);
            p2.y = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i + 1].y);
        ...
        case TT_PRIM_CSPLINE:
        {
          for (uint i = 0; i < (uint)pc->cpfx - 1; i++)
          {
            ...
            p3.x = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i + 2].x);
            p3.y = WinFontFace_getFloatFromFIXED<NumT>(pc->apfx[i + 2].y);
...
static err_t _WinGetKerningF(HDC hdc, FontKerningTableF** out)
{
  ...
  if (ok)
  {
    ...
    for (uint32_t i = 0; i < kerningPairsLength; i++)
    {
      ...
      // Font kerning information should be sorted, but it isn't. So uncomment
      // following code and set breakpoint to "NOT_SORTED" to catch the exact
      // place:
      //
      // fprintf(stderr, "[%X %X] - %f\n", chars.first, chars.second, amount);
      // if (chars.comparable < lastComparable)
      //   fprintf(stderr, "NOT_SORTED\n");
      
      isSorted &= chars.comparable >= lastComparable;
      lastComparable = chars.comparable;
...
err_t WinFontFace::_init(const LOGFONTW* logFont)
{
  ...
  if (FOG_IS_NULL(hFont))
    goto _EndDeleteDC;

  hFontOld = (HFONT)SelectObject(hDC, (HGDIOBJ)hFont);
  if (hFontOld == (HFONT)GDI_ERROR)
    goto _EndDeleteFont;

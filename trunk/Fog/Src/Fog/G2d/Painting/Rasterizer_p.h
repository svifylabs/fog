...
#include <Fog/Core/C++/PackRestore.h>
  ...
  //! @brief Scene-box in 24.8 format.
  BoxI _sceneBox24x8;
  ...
  //! @brief Cell chunks per rows. Index of value 0 is index to _sceneBox.y1.
  //!
  //! _rows can contain garbage in these indices:
  //! - _sceneBox.y0 -> _boundingBox.y0
  //! - _boundingBox.y1 -> _sceneBox.y1
  Row* _rowsStorage;

  //! @brief Adjusted _rowsStorage so index matches the position in the raster.
  //!
  //! @note It's extremely dangerouse to access this array out of sceneBox bounds,
  //! because it's adjusted and pointer is not valid outside of these bounds, so
  //! be careful when accessing it. The main purpose of this member is to prevent
  //! subtracting @c _sceneBox.y0 from @c _rowsStorage.
  Row* _rowsAdjusted;

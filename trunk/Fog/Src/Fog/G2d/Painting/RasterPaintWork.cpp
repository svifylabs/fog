...
void RasterPaintWorkTask::run()
{
  ...
  Logger::debug("Fog::RasterPaintWorkTask", "run",
    "Enter (ContextID=%d).", ctx.id);
    ...
    do {
      ...
      while (calcCurrent < wm->calcPosition)
      {
        ...
        if (clc && AtomicCore<RasterCalc*>::cmpXchg(pclc, clc, NULL))
        {
          ...
          Logger::debug("Fog::RasterPaintWorkTask", "run",
            "Run calculation %u (ContextID=%d).", (uint)calcCurrent, ctx.id);
      ...
      while (cmdCurrent < wm->cmdPosition)
      {
        ...
        Logger::debug("Fog::RasterPaintWorkTask", "run",
          "Run command %d, status=%s (ContextID=%d).", (int)cmdCurrent, statusMsg[cmd->status.get()], ctx.id);
#endif // FOG_DEBUG_RASTER_CMD
...
#if defined(FOG_DEBUG_RASTER_SYNC)
      Logger::debug("Fog::RasterPaintWorkTask", "run",
        "Going to wait, currently waiting=%d (ContextID=%d).", (int)wm->waitingWorkers.get(), ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
...
#if defined(FOG_DEBUG_RASTER_SYNC)
          Logger::debug("Fog::RasterPaintWorkTask", "run",
            "Everything done, signalling allFinished (ContextID=%d).", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
...
#if defined(FOG_DEBUG_RASTER_SYNC)
  Logger::debug("Fog::RasterPaintWorkTask", "destroy",
    "Enter (ContextID=%d)", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC
...
#if defined(FOG_DEBUG_RASTER_SYNC)
    Logger::debug("Fog::RasterPaintWorkTask", "destroy",
      "I'm last, signalling release event (ContextID=%d).", ctx.id);
#endif // FOG_DEBUG_RASTER_SYNC

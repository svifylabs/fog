...
_Repeat:
  ...
  if (nmemb > 7)
  {
    ...
    if (nmemb > 40)
    {
      size_t i = (nmemb / 8) * TypeT::_size;
      pl = _med3(pl        , pl + i, pl + 2 * i);
      pm = _med3(pm - i    , pm    , pm + i    );
      pn = _med3(pn - 2 * i, pn - i, pn        );
    }
  ...
  for (;;)
  {
    ...
    while (pb <= pc && (cmp = TypeT::_compare(pc, base)) >= 0)
    {
      if (cmp == 0)
      {
        swapFlag = 1;
  ...
  if (swapFlag)
  {
    ...
    // Step 1.
    i = Math::min<size_t>((size_t)(pa - base), (size_t)(pb - pa));
    swpA = base;
    swpB = pb - i;
    ...
    for (;;)
    {
      ...
      // Step 2.
      i = Math::min<size_t>((size_t)(pd - pc), (size_t)(pn - pd) - TypeT::_size);
      swpA = pb;
      swpB = pn - i;
    }

    // Recurse.
    if ((i = (size_t)(pb - pa)) > TypeT::_size)
      _sort(base, i / TypeT::_size);

    // Iterate.
    if ((i = (size_t)(pd - pc)) > TypeT::_size)
    {
      base = pn - i;
      nmemb = i / TypeT::_size;
      goto _Repeat;

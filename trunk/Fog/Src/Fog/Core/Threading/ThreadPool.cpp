...
static void FOG_CDECL ThreadPool_dtor(ThreadPool* self)
{
  ...
  if (used)
  {
    Logger::warning("Fog::ThreadPool", "~ThreadPool", "Waiting for threads whose are still in use...");
...
static err_t FOG_CDECL ThreadPool_releaseThreads(ThreadPool* self, Thread** threads, size_t count, int workerId)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    {
      Logger::error("Fog::ThreadPool", "releaseThreads",
        "Released thread (%p) which is not in a ThreadPool list.", t);

...
// ============================================================================
// [RPE (Raster Painter Engine) - Configuration]
// ============================================================================

// Maximum number of threads to use for rendering.
#define RPE_MAX_THREADS 16
// Maximum commands to accumulate in buffer.
#define RPE_MAX_COMMANDS 4096
// Minimum size to set multithreading on.
#define RPE_MIN_SIZE_THRESHOLD (256*256)

// #define RASTER_DEBUG

// ============================================================================
// [Raster Painter Engine - Forward Declarations]
// ============================================================================
...
struct FOG_HIDDEN RasterEngineThreadData
{
  ...
  Thread* threads[RPE_MAX_THREADS];
  Static<RasterEngineThreadTask> tasks[RPE_MAX_THREADS];

  // Commands
  Atomic<sysuint_t> commandsPosition;
  RasterEngineCommandAllocator commandAllocator;
  RasterEngineCommand* volatile commands[RPE_MAX_COMMANDS];
};
  ...
  _threadData(NULL)
{
  ...
  if (cpuInfo->numberOfProcessors > 1 && (hints & Painter::HintNoMultithreading) == 0)
  {
    ...
    if (total >= RPE_MIN_SIZE_THRESHOLD)
    {
#if defined(RASTER_DEBUG)
...
void RasterEngine::setMultithreaded(bool mt)
{
  ...
  if (mt)
  {
    int max = Math::min<int>(cpuInfo->numberOfProcessors, RPE_MAX_THREADS);
...
void RasterEngine::_postCommand(RasterEngineCommand* cmd)
{
  ...
  if (_threadData->commandsPosition.get() == RPE_MAX_COMMANDS)
  {
#if defined(RASTER_DEBUG)
...
{
  return new(std::nothrow) RasterEngine(pixels, width, height, stride, format, hints);
}

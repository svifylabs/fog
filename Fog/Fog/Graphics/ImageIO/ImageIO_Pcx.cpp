...
#include <Fog/Graphics/ImageIO/ImageIO_Pcx_p.h>
...
err_t PcxProvider::createDevice(uint32_t deviceType, BaseDevice** device) const
{
  ...
  switch (deviceType)
  {
    case IMAGE_IO_DEVICE_DECODER:
      d = fog_new PcxDecoderDevice(const_cast<PcxProvider*>(this));
      break;
    case IMAGE_IO_DEVICE_ENCODER:
      d = fog_new PcxEncoderDevice(const_cast<PcxProvider*>(this));
      break;
    default:
...
err_t PcxEncoderDevice::writeImage(const Image& image)
{
  ...
  if (image.getPalette().isAlphaUsed()) format = IMAGE_FORMAT_ARGB32;
  ...
  // Write 8 bit image.
  if (image.getDepth() == 8 && format != IMAGE_FORMAT_ARGB32)
  {
    for (y = 0; y != height; y++)
    {
  ...
  else if (image.getDepth() == 8 && format == IMAGE_FORMAT_ARGB32)
  {
...
{
  using namespace Fog;
  ImageIO::addProvider(IMAGE_IO_DEVICE_BOTH, fog_new ImageIO::PcxProvider());
}

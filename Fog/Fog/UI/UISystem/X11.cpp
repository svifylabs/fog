...
FOG_IMPLEMENT_OBJECT(Fog::X11UISystem)
FOG_IMPLEMENT_OBJECT(Fog::X11UIWindow)
...
#define UI_SYSTEM() \
  reinterpret_cast<X11UISystem*>(Application::getInstance()->getUiSystem())
...
// ============================================================================
// [Fog::X11UISystem - Functions in libX11, libXext, libXrender]
// ============================================================================
...
// ============================================================================
// [Fog::X11UISystem - Atoms]
// ============================================================================

// XAtom names, order specified in Wde/Gui/Application.h
static const char *X11_atomNames[X11UISystem::Atom_Count] =
{
  // WM events
  "WM_PROTOCOLS",                 /* 0  */
  "WM_DELETE_WINDOW",             /* 1  */
...
// ============================================================================
// [Fog::X11UISystem - Helpers]
// ============================================================================

static void X11UISystem_sendClientMessage(X11UISystem* uiSystem, XWindow win, long mask, long l0, long l1, long l2, long l3, long l4)
{
  XClientMessageEvent xe;

  xe.type = XClientMessage;
  xe.window = win;
  xe.message_type = uiSystem->getAtom(X11UISystem::Atom_WM_PROTOCOLS);
  xe.format = 32;
  xe.data.l[0] = l0;
  xe.data.l[1] = l1;
  xe.data.l[2] = l2;
...
// ============================================================================
// [Fog::X11UISystem - Update]
// ============================================================================

void X11UISystem::doUpdate()
{
  BaseUISystem::doUpdate();
  pXFlush(_display);
}

void X11UISystem::doBlitWindow(UIWindow* window, const Box* rects, sysuint_t count)
{
  reinterpret_cast<X11UIBackingStore*>(window->_backingStore)->blitRects(
    (XID)window->getHandle(), rects, count);
}

// ============================================================================
// [Fog::X11UISystem - X11 Keyboard]
// ============================================================================

void X11UISystem::initKeyboard()
{
...
uint32_t X11UISystem::translateXSym(KeySym xsym) const
{
  uint key = 0;
...
// ============================================================================
// [Fog::X11UISystem - X11 Mouse]
// ============================================================================

void X11UISystem::initMouse()
{
...
uint32_t X11UISystem::translateButton(uint x11Button) const
{
  switch (x11Button)
  {
    case 1:
...
// ============================================================================
// [Fog::X11UISystem - X11 ColorMap]
// ============================================================================

bool X11UISystem::createColormap()
{
...
uint X11UISystem::allocRGB(XColormap colormap, int nr, int ng, int nb, uint8_t* palconv)
{
  int r, g, b, i;
  int sig_mask = 0;
...
void X11UISystem::freeRGB(XColormap colormap, const uint8_t* palconv, uint count)
{
  if (count == 0) return;
...
// ============================================================================
// [Fog::X11UISystem - UIWindow]
// ============================================================================

UIWindow* X11UISystem::createUIWindow(Widget* widget)
{
  return new(std::nothrow) X11UIWindow(widget);
}

void X11UISystem::destroyUIWindow(UIWindow* native)
{
  delete native;
}

// ============================================================================
// [Fog::X11UISystem - X11 API]
// ============================================================================

err_t X11UISystem::loadLibraries()
{
  // Load X11.
...
// ============================================================================
// [Fog::X11UISystem - WM Support]
// ============================================================================

XID X11UISystem::getWmClientLeader()
{
...
X11UIWindow::X11UIWindow(Widget* widget) :
  BaseUIWindow(widget),
  _xic(0),
  _inputOnly(false),
  _mapRequest(false),
  _xflags(0)
{
  _backingStore = new(std::nothrow) X11UIBackingStore();
}

X11UIWindow::~X11UIWindow()
{
  destroy();
  delete _backingStore;
}

err_t X11UIWindow::create(uint32_t createFlags)
{
  if (_handle) return ERR_UI_WINDOW_ALREADY_EXISTS;
  ...
  // Window protocols.
  {
    ...
    // NET_WM_PING support.
    protocols[1] = atoms[X11UISystem::Atom_NET_WM_PING];
  ...
  // Set _NET_WM_PID
  long pid = getpid();
  uiSystem->pXChangeProperty(display, (XID)getHandle(), atoms[X11UISystem::Atom_NET_WM_PID], XA_CARDINAL, 32, PropModeReplace, (unsigned char*)&pid, 1);

  // Set WM_CLIENT_LEADER
  long clientLeader = uiSystem->getWmClientLeader();
  uiSystem->pXChangeProperty(display, (XID)getHandle(), atoms[X11UISystem::Atom_WM_CLIENT_LEADER], XA_WINDOW, 32, PropModeReplace, (unsigned char*)&clientLeader, 1);
...
err_t X11UIWindow::move(const Point& pt)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::resize(const Size& size)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::reconfigure(const Rect& rect)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::setTitle(const String& title)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
  ...
  {
    err = ERR_UI_INTERNAL_ERROR;
  }

  return err;
}

err_t X11UIWindow::getTitle(String& title)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::setIcon(const Image& icon)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::getSizeGranularity(Point& pt)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11UIWindow::worldToClient(Point* coords)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
  ...
  return (ok) ? (err_t)ERR_OK : (err_t)ERR_UI_CANT_TRANSLETE_COORDINATES;
}

err_t X11UIWindow::clientToWorld(Point* coords)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
void X11UIWindow::onX11Event(XEvent* xe)
{
  X11UISystem* uiSystem = UI_SYSTEM();
    ...
    case XExpose:
    {
      if (!_isDirty)
      {
        do {
          ...
          reinterpret_cast<X11UIBackingStore*>(_backingStore)->blitRects(
            (XID)getHandle(), &box, 1);
    ...
    case XClientMessage:
    {
      ...
      if (xe->xclient.format == 32)
      {
        ...
        if (xe->xclient.message_type == uiSystem->getAtom(X11UISystem::Atom_WM_PROTOCOLS))
        {
          ...
          else if (msg == uiSystem->getAtom(X11UISystem::Atom_NET_WM_PING))
          {
            if (xe->xclient.window != uiSystem->_root)
            {
              X11UISystem_sendClientMessage(
...
void X11UIWindow::setMoveableHints()
{
  XSizeHints hints;
  Memory::zero(&hints, sizeof(XSizeHints));
...
X11UIBackingStore::X11UIBackingStore()
{
  _pixmap = 0;
...
bool X11UIBackingStore::resize(int width, int height, bool cache)
{
  X11UISystem* uiSystem = UI_SYSTEM();
...
void X11UIBackingStore::destroy()
{
  resize(0, 0, false);
}

void X11UIBackingStore::updateRects(const Box* rects, sysuint_t count)
{
  X11UISystem* uiSystem = UI_SYSTEM();
...
void X11UIBackingStore::blitRects(XID target, const Box* rects, sysuint_t count)
{
  X11UISystem* uiSystem = UI_SYSTEM();
  sysuint_t i;
...
X11EventLoop::X11EventLoop() : EventLoop(Ascii8("UI.X11"))
{
  _wakeUpSent.init(0);
}
...
void X11EventLoop::_runInternal()
{
  static const sysuint_t WORK_PER_LOOP = 2;

  sysuint_t work = 0;

  // Inspired in WinEventLoop and ported to X11EventLoop.
  for (;;)
  {
    bool didWork = false;
    bool more;
...
void X11EventLoop::_scheduleWork()
{
  _sendWakeUp();
}

void X11EventLoop::_scheduleDelayedWork(const Time& delayedWorkTime)
{
...
void X11EventLoop::_waitForWork()
{
  X11UISystem* uiSystem = UI_SYSTEM();
  ...
  if (ret < 0)
  {
    fog_debug("Fog::X11EventLoop::waitForWork() - select() failed (errno=%d).", errno);
  }
  ...
  if (ret > 0)
  {
    if (FD_ISSET(uiSystem->_wakeUpPipe[0], &fdSet))
    {
      ...
      {
        fog_debug("Fog::X11EventLoop::waitForWork() - Can't read from weak-up pipe.");
      }
...
bool X11EventLoop::_processNextXEvent()
{
  X11UISystem* uiSystem = UI_SYSTEM();
  ...
  X11UIWindow* uiWindow = reinterpret_cast<X11UIWindow*>(uiSystem->handleToNative((void*)xe.xany.window));
...
void X11EventLoop::_sendWakeUp()
{
  if (_wakeUpSent.cmpXchg(0, 1))
  {
    ...
    {
      {
        fog_debug("Fog::X11EventLoop::sendWakeUp() - Can't write to weak-up pipe");
      }
    }
  }
}
...
{
  return new (std::nothrow) Fog::X11UISystem();
}

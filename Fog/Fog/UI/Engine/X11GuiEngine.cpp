...
#include <Fog/UI/Engine/X11GuiEngine.h>
...
{
  Debug::failFormat("Fog::X11GuiEngine::IOErrorHandler() - Fatal error");
...
static int X11_ErrorHandler(XDisplay* d, XErrorEvent* e)
{
  ...
  Debug::dbgFunc("Fog::UIsystemX11", "ErrorHandler", "%s", buffer);
  return 0;
}
...
X11GuiEngine::X11GuiEngine()
{
  ...
  {
    Debug::dbgFunc("Fog::X11GuiEngine", "X11GuiEngine", "Can't load X11 libraries.");
    return;
  }
  ...
  if (!pXSupportsLocale())
  {
    Debug::dbgFunc("Fog::X11GuiEngine", "X11GuiEngine", "X does not support locale.");
  }
  else
  {
    ...
    {
      Debug::dbgFunc("Fog::X11GuiEngine", "X11GuiEngine", "Can't set X locale modifiers.");
    }
  }
...
err_t X11GuiWindow::setPosition(const PointI& pos)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11GuiWindow::setSize(const SizeI& size)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
err_t X11GuiWindow::setGeometry(const RectI& rect)
{
  if (!_handle) return ERR_RT_INVALID_HANDLE;
...
void X11GuiWindow::onX11Event(XEvent* xe)
{
  ...
  {
    case XCreateNotify:
      Debug::dbgFormat("Fog::X11GuiWindow::onX11Event() - Create notify");
      break;
    case XDestroyNotify:
      Debug::dbgFormat("Fog::X11GuiWindow::onX11Event() - Destroy notify");
      break;
    ...
    case XConfigureRequest:
      Debug::dbgFormat("Fog::X11GuiWindow::onX11Event() - Configure request: %d %d", xe->xconfigurerequest.x, xe->xconfigurerequest.y);
      break;
      ...
      if (_xic)
      {
        ...
        switch (status)
        {
          case XBufferOverflow:
            Debug::dbgFormat("Fog::X11GuiWindow::onX11Event() - 'KeyPress', Buffer too small (XIC)");
...
bool X11GuiBackBuffer::resize(int width, int height, bool cache)
{
  ...
  if (createImage)
  {
    ...
    if (engine->hasXShm())
    {
      ...
      {
        Debug::dbgFunc("Fog::X11GuiBackBuffer", "resize", "shmget() failed: %s", strerror(errno));
        goto __tryImage;
      }

      if ((_shmi.shmaddr = (char *)shmat(_shmi.shmid, NULL, 0)) == NULL)
      {
        Debug::dbgFunc("Fog::X11GuiBackBuffer", "resize", "shmat() failed: %s", strerror(errno));
        shmctl(_shmi.shmid, IPC_RMID, NULL);
        goto __tryImage;
      }
...
__tryImage:
    ...
    if (_type != TYPE_NONE)
    {
      ...
      if (targetStride != secondaryStride)
      {
        ...
        if (!_secondaryPixels)
        {
          Debug::dbgFunc("Fog::X11GuiBackBuffer", "resize", "Can't create secondary backing store buffer");
        }
        ...
        if (!_convertFunc)
        {
          Debug::dbgFunc("Fog::X11GuiBackBuffer", "resize", "Not available converter for %d bit depth", targetDepth);
        }
...
void X11GuiBackBuffer::updateRects(const BoxI* rects, size_t count)
{
  ...
  if (_secondaryPixels && _convertFunc)
  {
    ...
    for (size_t i = 0; i != count; i++)
    {
      ...
      uint8_t* dstCur = dstBase + y0 * dstStride + x0 * dstBPP;
      uint8_t* srcCur = srcBase + y0 * srcStride + x0 * srcBPP;

      while (y0 < y1)
      {
        // TODO: X11 Engine.
        _convertFunc(dstCur, srcCur, w, NULL);
...
void X11GuiEventLoop::_waitForWork()
{
  ...
  if (ret < 0)
  {
    Debug::dbgFormat("Fog::X11GuiEventLoop::waitForWork() - select() failed (errno=%d).", errno);
  }
  ...
  if (ret > 0)
  {
    if (FD_ISSET(engine->_wakeUpPipe[0], &fdSet))
    {
      ...
      {
        Debug::dbgFormat("Fog::X11GuiEventLoop::waitForWork() - Can't read from weak-up pipe.");
      }
...
void X11GuiEventLoop::_sendWakeUp()
{
  if (_wakeUpSent.cmpXchg(0, 1))
  {
    ...
    {
      {
        Debug::dbgFormat("Fog::X11GuiEventLoop::sendWakeUp() - Can't write to weak-up pipe");
      }
    }
  }
}

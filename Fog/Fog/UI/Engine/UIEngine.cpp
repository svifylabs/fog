  ...
  const RectI& clientGeometry)
{
  ...
  if (windowGeometry.x != d->_windowGeometry.x || windowGeometry.y != windowGeometry.y)
    geometryFlags |= UI_ENGINE_CHANGED_WINDOW_POSITION;
  if (windowGeometry.w != d->_windowGeometry.w || windowGeometry.h != windowGeometry.h)
    geometryFlags |= UI_ENGINE_CHANGED_WINDOW_SIZE;
  if (clientGeometry.x != d->_clientGeometry.x || clientGeometry.y != clientGeometry.y)
    geometryFlags |= UI_ENGINE_CHANGED_CLIENT_POSITION;
  if (clientGeometry.w != d->_clientGeometry.w || clientGeometry.h != clientGeometry.h)
  ...
  scheduleWindowUpdate(window, clientGeometry);
}
...
{
  UIEngineWindowImpl* d = window->_d;
  d->_regionOfInterest.union_(rect);
...
{
  UIEngineWindowImpl* d = window->_d;
  d->_regionOfInterest.union_(region);
...
void UIEngine::doUpdateAll()
{
  ...
  for (it.start(); it.isValid(); it.next())
  {
    ...
    RectI boundingBox(d->_regionOfInterest.getBoundingBox());
    err_t err;

...
static FOG_INLINE err_t integrate(float* dst, const MathFunctionF& f, const IntervalF& interval, uint32_t method, uint32_t steps)
{
...
//! @overload
static FOG_INLINE err_t integrate(double* dst, const MathFunctionD& f, const IntervalD& interval, uint32_t method, uint32_t steps)
{
  FOG_ASSERT(method < MATH_INTEGRATION_METHOD_COUNT);

...
struct FOG_NO_EXPORT CircleF
{
  ...
  FOG_INLINE void setCircle(float x0, float y0, float rad) { center.set(x0, y0); radius = rad; }
  ...
  {
    return fog_api.circlef_getBoundingBox(this, &dst, tr);
  }

  FOG_INLINE err_t _getBoundingRect(RectF& dst, const TransformF* tr) const
  {
    err_t err = fog_api.circlef_getBoundingBox(this, reinterpret_cast<BoxF*>(&dst), tr);
    dst.w -= dst.x;
    dst.h -= dst.y;
    return err;
  }
  ...
  {
    return fog_api.circlef_hitTest(this, &pt);
  }
  ...
  {
    return fog_api.circlef_toCSpline(this, dst);
  }
...
struct FOG_NO_EXPORT CircleD
{
  ...
  FOG_INLINE void setCircle(double x0, double y0, double r) { center.set(x0, y0); radius = r; }
  ...
  {
    return fog_api.circled_getBoundingBox(this, &dst, tr);
  }

  FOG_INLINE err_t _getBoundingRect(RectD& dst, const TransformD* tr) const
  {
    err_t err = fog_api.circled_getBoundingBox(this, reinterpret_cast<BoxD*>(&dst), tr);
    dst.w -= dst.x;
    dst.h -= dst.y;
    return err;
  }
  ...
  {
    return fog_api.circled_hitTest(this, &pt);
  }
  ...
  {
    return fog_api.circled_toCSpline(this, dst);
  }

...
struct FOG_NO_EXPORT FreeTypeLibrary
{
  FreeTypeLibrary();
  ~FreeTypeLibrary();
...
err_t FreeTypeLibrary::init()
{
  ...
  if (dll.getSymbols(addr, symbols, FOG_ARRAY_SIZE(symbols), NUM_SYMBOLS, (char**)&badSymbol) != NUM_SYMBOLS)
  {
    ...
    Debug::dbgFunc("Fog::FreeTypeLibrary", "init", "Can't load symbol '%s'.\n", badSymbol);
  ...
  {
    Debug::dbgFunc("Fog::FreeTypeLibrary", "init", "Can't init FreeType, FT_Error: %u.\n", error);
...
#if defined(FOG_HAVE_FONTCONFIG)
struct FOG_NO_EXPORT FontConfigLibrary
{
  FontConfigLibrary();
  ~FontConfigLibrary();
...
err_t FontConfigLibrary::init()
{
  ...
  {
    // Some symbol failed to load? Inform about it.
    Debug::dbgFunc("Fog::FontConfigLibrary", "init", "Can't load symbol '%s'.\n", badSymbol);
    dll.close();
    return ERR_FONT_FONTCONFIG_NOT_LOADED;
  }
  ...
  if (!pFcInit())
  {
    Debug::dbgFunc("Fog::FontConfigLibrary", "init", "Can't init FontConfig.\n");
    dll.close();
...
struct FOG_NO_EXPORT FontTranslatorRecord
{
...
struct FOG_NO_EXPORT FontTranslator
{
  FontTranslator();
  ~FontTranslator();
...
struct FOG_NO_EXPORT FTFontFile
{
...
FTFontEngine::FTFontEngine() : FontEngine(FONT_FACE_FREETYPE)
{
  ...
  if (FOG_IS_ERROR(err))
  {
    Debug::dbgFunc("Fog::FTFontEngine", "FTFontEngine", "Can't load or initialize FreeType, error %u.\n", err);
  }
}
...
void FTFontEngine::close()
{
  ...
  for (it.toStart(); it.isValid(); it.remove())
  {
    ...
    {
      Debug::dbgFunc("Fog::FTFontEngine", "close", "FTFile not dereferenced, refCount=%ld.\n", (long)file->refCount.get());
    }
...
static err_t decompose_ft_glyph_outline(const FT_Outline& outline, bool flipY, const TransformD* mtx, PathD& path)
{
  ...
  for (n = 0; n < outline.n_contours; n++)
  {
    ...
    if (tag == FT_CURVE_TAG_CUBIC) { err = ERR_FONT_INVALID_DATA; goto _End; }
    ...
    x0 = fx26p6ToDouble(v_start.x);
    y0 = fx26p6ToDouble(v_start.y);
    if (flipY) y0 = -y0;
    if ((err = path.moveTo(PointD(x0, y0)))) goto _End;
    ...
    while (point < limit)
    {
      ...
      switch(tag)
      {
        ...
        {
          x0 = fx26p6ToDouble(point->x);
          y0 = fx26p6ToDouble(point->y);
          if (flipY) y0 = -y0;
          if ((err = path.lineTo(PointD(x0, y0)))) goto _End;
          continue;
        }
...
_DoConic:
          if (point < limit)
          {
            ...
            {
              x0 = fx26p6ToDouble(v_control.x);
              y0 = fx26p6ToDouble(v_control.y);
              x1 = fx26p6ToDouble(vec.x);
              y1 = fx26p6ToDouble(vec.y);
              if (flipY) { y0 = -y0; y1 = -y1; }
              if ((err = path.quadTo(PointD(x0, y0), PointD(x1, y1)))) goto _End;
              continue;
            }

            if (tag != FT_CURVE_TAG_CONIC) { err = ERR_FONT_INVALID_DATA; goto _End; }
            ...
            x0 = fx26p6ToDouble(v_control.x);
            y0 = fx26p6ToDouble(v_control.y);
            x1 = fx26p6ToDouble(v_middle.x);
            y1 = fx26p6ToDouble(v_middle.y);
            if (flipY) { y0 = -y0; y1 = -y1; }
            if ((err = path.quadTo(PointD(x0, y0), PointD(x1, y1)))) goto _End;
          ...
          if (flipY) { y0 = -y0; y1 = -y1; }
          path.quadTo(PointD(x0, y0), PointD(x1, y1));
          goto _DoClose;
        }
        ...
        case FT_CURVE_TAG_CUBIC:
        {
          ...
          if (point <= limit)
          {
            ...
            x0 = fx26p6ToDouble(vec0.x);
            y0 = fx26p6ToDouble(vec0.y);
            x1 = fx26p6ToDouble(vec1.x);
            y1 = fx26p6ToDouble(vec1.y);
            x2 = fx26p6ToDouble(vec.x);
            y2 = fx26p6ToDouble(vec.y);

            if (flipY) { y0 = -y0; y1 = -y1; y2 = -y2; }
            if ((err = path.cubicTo(PointD(x0, y0), PointD(x1, y1), PointD(x2, y2)))) goto _End;
            continue;
          }
...
GlyphData* FTFontFace::renderGlyph(uint32_t uc)
{
  ...
  // Get glyph index.
  if ((index = _freeTypeLib->pFT_Get_Char_Index(face, uc)) == 0) goto _End;
  // Try to load the glyph.
  if ((error = _freeTypeLib->pFT_Load_Glyph(face, index, FT_LOAD_DEFAULT)) != 0) goto _End;
...

  {
    ...
    */
    ...
    if (glyphd == NULL) goto _End;

    if (width != 0 && glyphd->bitmap.create(SizeI(width, slot->bitmap.rows), IMAGE_FORMAT_A8) != ERR_OK)
    {
      fog_delete(glyphd);
      glyphd = NULL;
      goto _End;
...
FTFontFile::FTFontFile(const String& fileName, const String& family) :
  ...
  fixedWidthIndex(0)
{
  //Debug::dbgFormat("Fog::FTFontFile::new()\n");
  refCount.init(0);
  used.init(0);
...
{
  // Debug::dbgFormat("Fog::FTFontFile::delete()\n");
...
{
  //Debug::dbgFormat("Fog::FTFontFile::ref() - RefCount=%lld\n", (long long)refCount.get());
  refCount.inc();;
  return this;
}
...
{
  //Debug::dbgFormat("Fog::FTFontFile::deref() - RefCount=%lld\n", (long long)refCount.get());
  refCount.dec();
}
...
bool FTFontFile::use()
{
  //Debug::dbgFormat("Fog::FTFontFile::use()\n");
  used.inc();
...
{
  //Debug::dbgFormat("Fog::FTFontFile::unuse()\n");

...
static int WinUIEngine_getMouseWheelLines(WinUIEngine* self)
{
  FOG_UNUSED(self);
...
WinUIEngine::WinUIEngine()
{
  ...
  _eventLoop.adopt(fog_new WinUIEventLoopImpl());
  _isInitialized = true;
}
...
void WinUIEngine::initMouse()
{
  ...
  _mouseInfo.setWheelLines(WinUIEngine_getMouseWheelLines(this));
  ...
  switch (vk)
  {
    ...
    default:
    {
      ...
      if (count == 1 && unicode[0] > 31)
        return unicode[0];
      if (count == 2)
        return CharW::ucs4FromSurrogate(
          static_cast<uint16_t>(unicode[0]),
          static_cast<uint16_t>(unicode[1]));
...
void WinUIEngine::doBlitWindow(UIEngineWindow* window)
{
  ...
  BoxI bBox = d->_blitRegion.getBoundingBox();

  if (d->_blitRegion.getLength() > 1)
  {
    HRGN hRgn;

    if (d->_blitRegion.toHRGN(&hRgn) == ERR_OK)
    {
...
{
  WinUIEngineWindowImpl* wImpl = fog_new WinUIEngineWindowImpl(this, window);
  if (FOG_IS_NULL(wImpl))
...
{
  WinUIEngineWindowImpl* wImpl = static_cast<WinUIEngineWindowImpl*>(window->_d);
  if (FOG_IS_NULL(wImpl))

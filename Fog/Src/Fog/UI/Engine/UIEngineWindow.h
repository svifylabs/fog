...
struct FOG_API UIEngineWindowImpl
{
  ...
  //! @brief Create a native @ref UIEngineWindow and assign it to @c _handle
  //! member.
  virtual err_t create(uint32_t hints) = 0;

  //! @brief Destroy the native @ref UIEngineWindow and call @c destroyed().
  virtual err_t destroy() = 0;

  //! @brief Called by @c destroy() after the window was destroyed.
  //!
  //! The purpose of this method is to reset all members to the default state
  ...
  virtual err_t setWindowOpacity(float opacity) = 0;
  ...
  //! @brief Window is being destroyed.
  uint32_t _windowBeingDestroyed : 1;
  
  //! @brief Reserved flags.
  uint32_t _windowReserved : 31;
  ...
  //! @brief Paint region (used to clip painter when invoking paint event).
  Region _paintRegion;
  //! @brief Blit region (used to blit window content to the screen).
  Region _blitRegion;
...
struct FOG_API UIEngineWindow
{
  ...
  //! @brief Set the opacity of the UIEngineWindow.
  err_t setWindowOpacity(float opacity);

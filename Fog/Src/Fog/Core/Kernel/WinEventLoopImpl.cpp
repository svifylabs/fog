...
void WinEventLoopImpl::scheduleDelayedWork(const Time& delayedWorkTime)
{
  ...
  ::SetTimer(_messageHwnd, reinterpret_cast<UINT_PTR>(this), delay, NULL);
}
...
bool WinEventLoopImpl::processMessageHelper(const MSG& msg)
{
  ...
  FOG_EVENT_LOOP_OBSERVER_LIST_EACH(_observerList, onBeforeDispatch((void*)&msg));

  ::TranslateMessage(&msg);
  ::DispatchMessageW(&msg);

  FOG_EVENT_LOOP_OBSERVER_LIST_EACH(_observerList, onAfterDispatch((void*)&msg));
...
bool WinEventLoopImpl::processPumpReplacementMessage()
{
  ...
  MSG msg;
  bool haveMessage = (0 != ::PeekMessageW(&msg, NULL, 0, 0, PM_REMOVE));

  FOG_ASSERT(!haveMessage ||
  ...
  return haveMessage && processMessageHelper(msg);
}
...
int WinEventLoopImpl::getCurrentDelay() const
{
  ...
  int64_t delta = (_delayedWorkTime - Time::now()).getDelta();

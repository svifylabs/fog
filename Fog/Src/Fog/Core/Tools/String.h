...
struct FOG_NO_EXPORT StringW
{
  ...
  FOG_INLINE void _modified(uint16_t* end)
  {
    _modified(reinterpret_cast<CharW*>(end));
  }
  ...
  FOG_INLINE bool isCanonical(uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL) const
  {
    return fog_api.stringw_isCanonical(this, NULL, version, where);
  }

  FOG_INLINE bool isCanonical(const Range& range, uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL) const
  {
    return fog_api.stringw_isCanonical(this, &range, version, where);
  }

  FOG_INLINE err_t makeCanonical(uint32_t version = CHAR_UNICODE_VERSION_DEFAULT)
  {
    return fog_api.stringw_makeCanonical(this, NULL, version);
  }

  FOG_INLINE err_t makeCanonical(const Range& range, uint32_t version = CHAR_UNICODE_VERSION_DEFAULT)
  {
    return fog_api.stringw_makeCanonical(this, &range, version);
  }
  ...
  FOG_INLINE bool isNormalized(uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL)
  {
    return fog_api.stringw_isNormalized(this, NULL, normForm, version, where);
  }

  FOG_INLINE bool isNormalized(const Range& range, uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL)
  {
    return fog_api.stringw_isNormalized(this, &range, normForm, version, where);
  }

  FOG_INLINE uint32_t quickCheck(uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL)
  {
    return fog_api.stringw_quickCheck(this, NULL, normForm, version, where);
  }

  FOG_INLINE uint32_t quickCheck(const Range& range, uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT, size_t* where = NULL)
  {
    return fog_api.stringw_quickCheck(this, &range, normForm, version, where);
  }

  FOG_INLINE err_t normalize(uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT)
  {
    return fog_api.stringw_normalize(this, NULL, normForm, version);
  }

  FOG_INLINE err_t normalize(const Range& range, uint32_t normForm,
    uint32_t version = CHAR_UNICODE_VERSION_DEFAULT)
  {
    return fog_api.stringw_normalize(this, &range, normForm, version);
  }

...
FOG_NO_EXPORT void ThreadLocal_init(void)
{
  ...
  {
    Logger::fatal("Fog::ThreadLocal", "$init", "Failed to call TlsAlloc(), TLS_OUT_OF_INDEXES returned.");
...
FOG_NO_EXPORT void ThreadLocal_init(void)
{
  ...
  {
    Logger::fatal("Fog::ThreadLocal", "$init", "Failed to call pthread_key_create(), errno=%d.", errno);
    Application::terminate(-1);
  }

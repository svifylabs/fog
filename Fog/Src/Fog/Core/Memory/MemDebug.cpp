...
void MemDbg::remove(void* p)
{
  ...
  if (!rbRemove(node))
  {
    Logger::fatal("Fog::MemDbg", "remove", "Pointer %p not found.", _USER_FROM_DBG(p));
...
void MemDbg::_check(void* p)
{
  ...
  if (!rbCheck(node))
  {
    Logger::fatal("Fog::MemDbg", "_check", "Pointer %p not found.", _USER_FROM_DBG(p));
    Application::terminate(-1);
  }
  ...
  {
    Logger::fatal("Fog::MemDbg", "_check", "Detected buffer underrun (%p).", _USER_FROM_DBG(p));
    Application::terminate(-1);
  }
  ...
  {
    Logger::fatal("Fog::MemDbg", "_check", "Detected buffer overrun (%p).", _USER_FROM_DBG(p));
    Application::terminate(-1);
  }
}
...
void MemDbg::show()
{
  ...
  Logger::error("Fog::MemDbg", "show", "Detected %u memory leaks (%u bytes total)", (uint)blocksUsed, (uint)bytesUsed);
}

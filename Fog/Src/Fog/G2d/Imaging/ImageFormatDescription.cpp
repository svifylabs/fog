...
static err_t ImageFormatDescription_detectIntegerFormatValues(ImageFormatDescription* self)
{
  ...
  if (self->_depth == 24 && self->_isByteSwapped && self->_hasAlignedComponents)
  {
    ...
    self->_rMask = MemOps::bswap32((uint32_t)self->_rMask) >> 8;
    self->_gMask = MemOps::bswap32((uint32_t)self->_gMask) >> 8;
    self->_bMask = MemOps::bswap32((uint32_t)self->_bMask) >> 8;
  }
  ...
  if (self->_depth == 32 && self->_isByteSwapped && self->_hasAlignedComponents)
  {
    ...
    self->_aMask = MemOps::bswap32((uint32_t)self->_aMask);
    self->_rMask = MemOps::bswap32((uint32_t)self->_rMask);
    self->_gMask = MemOps::bswap32((uint32_t)self->_gMask);
    self->_bMask = MemOps::bswap32((uint32_t)self->_bMask);
  }

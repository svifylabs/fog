...
static void _BmpSwapFileHeader(BmpFileHeader* h)
{
  ...
  h->fileSize                  = MemOps::bswap32(h->fileSize);
  h->imageOffset               = MemOps::bswap32(h->imageOffset);
...
static void _BmpSwapDataHeader(BmpDataHeader* h, uint32_t headerSize)
{
  ...
  switch (headerSize)
  {
    ...
    {
      h->os2v1.width           = MemOps::bswap16le(h->os2v1.width);
      h->os2v1.height          = MemOps::bswap16le(h->os2v1.height);
      h->os2v1.planes          = MemOps::bswap16le(h->os2v1.planes);
      h->os2v1.bitsPerPixel    = MemOps::bswap16le(h->os2v1.bitsPerPixel);
      break;
    }

    case BMP_HEADER_SIZE_WIN_V5:
      h->winv5.intent          = MemOps::bswap32le(h->winv5.intent);
      h->winv5.profileData     = MemOps::bswap32le(h->winv5.profileData);
      h->winv5.profileSize     = MemOps::bswap32le(h->winv5.profileSize);
      // ... Fall through ...

    case BMP_HEADER_SIZE_WIN_V4:
      h->winv4.rMask           = MemOps::bswap32le(h->winv4.rMask);
      h->winv4.gMask           = MemOps::bswap32le(h->winv4.gMask);
      h->winv4.bMask           = MemOps::bswap32le(h->winv4.bMask);
      h->winv4.aMask           = MemOps::bswap32le(h->winv4.aMask);
      h->winv4.colorspace      = MemOps::bswap32le(h->winv4.colorspace);
      h->winv4.rX              = MemOps::bswap32le(h->winv4.rX);
      h->winv4.rY              = MemOps::bswap32le(h->winv4.rY);
      h->winv4.rZ              = MemOps::bswap32le(h->winv4.rZ);
      h->winv4.gX              = MemOps::bswap32le(h->winv4.gX);
      h->winv4.gY              = MemOps::bswap32le(h->winv4.gY);
      h->winv4.gZ              = MemOps::bswap32le(h->winv4.gZ);
      h->winv4.bX              = MemOps::bswap32le(h->winv4.bX);
      h->winv4.bY              = MemOps::bswap32le(h->winv4.bY);
      h->winv4.bZ              = MemOps::bswap32le(h->winv4.bZ);
      h->winv4.rGamma          = MemOps::bswap32le(h->winv4.rGamma);
      h->winv4.gGamma          = MemOps::bswap32le(h->winv4.gGamma);
      h->winv4.bGamma          = MemOps::bswap32le(h->winv4.bGamma);
      // ... Fall through ...

    case BMP_HEADER_SIZE_WIN_V3:
      h->winv3.width           = MemOps::bswap32le(h->winv3.width);
      h->winv3.height          = MemOps::bswap32le(h->winv3.height);
      h->winv3.planes          = MemOps::bswap16le(h->winv3.planes);
      h->winv3.bitsPerPixel    = MemOps::bswap16le(h->winv3.bitsPerPixel);
      h->winv3.compression     = MemOps::bswap32le(h->winv3.compression);
      h->winv3.imageSize       = MemOps::bswap32le(h->winv3.imageSize);
      h->winv3.horzResolution  = MemOps::bswap32le(h->winv3.horzResolution);
      h->winv3.vertResolution  = MemOps::bswap32le(h->winv3.vertResolution);
      h->winv3.colorsUsed      = MemOps::bswap32le(h->winv3.colorsUsed);
      h->winv3.colorsImportant = MemOps::bswap32le(h->winv3.colorsImportant);
      break;
  }
#endif
}
...
err_t BmpDecoder::readHeader()
{
  ...
  if (_stream.read(&bmpDataHeader.headerSize, 4) != 4)
    return (_headerResult = ERR_IMAGE_TRUNCATED);
  headerSize = MemOps::bswap32le(bmpDataHeader.headerSize);
...
#else
        for (i = 0; i != nColors; i++) pal[i] = MemOps::bswap32(pal[i]) | 0xFF000000;
#endif
      }
      ...
      (_depth == 16 || _depth == 32))
  {
    ...
    bitFieldMask[0] = MemOps::bswap32le(bitFieldMask[0]);
    bitFieldMask[1] = MemOps::bswap32le(bitFieldMask[1]);
    bitFieldMask[2] = MemOps::bswap32le(bitFieldMask[2]);

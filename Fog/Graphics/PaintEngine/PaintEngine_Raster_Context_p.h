...
// [Guard]
#ifndef _FOG_GRAPHICS_PAINTENGINE_RASTER_CONTEXT_P_H
#define _FOG_GRAPHICS_PAINTENGINE_RASTER_CONTEXT_P_H
...
struct FOG_HIDDEN RasterPaintContext
{
  ...
  //! @brief Get context buffer.
  //!
  //! Each paint context (main or per worker) contains one buffer that can
  //! be retrieved by @c getBuffer() method. There is no method to release
  //! that buffer, because it's simply returned again and again. The pointer
  //! is valid to the next @c getBuffer() call.
  //!
  //! Buffer is normally freed by the @c RasterPaintContext destructor.
  FOG_INLINE uint8_t* getBuffer(sysuint_t size);

  //! @brief Realloc context buffer.
  //!
  //! You should never call this function, use @c getBuffer() instead.
...
// [Guard]
#endif // _FOG_GRAPHICS_PAINTENGINE_RASTER_CONTEXT_P_H
